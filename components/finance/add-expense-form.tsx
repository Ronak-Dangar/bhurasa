...
const expenseCategories = [
  { value: "purchase_groundnuts", label: "Purchase Groundnuts", procurement: true, autoItem: true },
  { value: "purchase_packaging", label: "Purchase Packaging/Other", procurement: true, autoItem: false },
  { value: "transport", label: "Transport", procurement: false, autoItem: false },
  { value: "labor", label: "Labor", procurement: false, autoItem: false },
  { value: "maintenance", label: "Maintenance", procurement: false, autoItem: false },
  { value: "utilities", label: "Utilities", procurement: false, autoItem: false },
];
...
export function AddExpenseForm() {
  ...
  const categoryConfig = expenseCategories.find(c => c.value === selectedCategory);
  const isProcurement = categoryConfig?.procurement || false;
  const isAutoItem = categoryConfig?.autoItem || false;

  useEffect(() => {
    // Only fetch inventory items for packaging purchases
    if (isProcurement && !isAutoItem && isOpen) {
      fetch('/api/inventory-items?type=packaging')
        .then(res => res.json())
        .then(data => setInventoryItems(data.items || []))
        .catch(() => setInventoryItems([]));
    }
  }, [isProcurement, isAutoItem, isOpen]);
        ...
        {isProcurement && (
          <div className="rounded-lg border border-emerald-200 bg-emerald-50/60 p-4 dark:border-emerald-900/40 dark:bg-emerald-950/30">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {!isAutoItem && (
                <div>
                  ...
                  <select
                    ...
                    required={isProcurement && !isAutoItem}
              ...
              <div className={isAutoItem ? "md:col-span-2" : ""}>
                <label className="block text-sm font-medium text-emerald-900 dark:text-emerald-100">
                  {isAutoItem ? "Quantity (kg)" : "Quantity Purchased (units)"}
                </label>
                <input
                  type="number"
                  ...
                  placeholder={isAutoItem ? "e.g., 1000 kg" : "e.g., 500 bottles"}
                  className="mt-1 w-full rounded-lg border border-emerald-300 bg-white px-3 py-2 text-sm text-zinc-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-200 dark:border-emerald-700 dark:bg-zinc-900 dark:text-zinc-100"
                />
              </div>
            </div>
            <p className="mt-2 text-xs text-emerald-700 dark:text-emerald-300">
              {isAutoItem 
                ? "ℹ️ Groundnut stock will be automatically updated with WAC calculation" 
